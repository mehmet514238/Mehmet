...
export function aCheck(flag = false) {
    ...
    if(currentTime >= global.resetTime || flag) {
        try {
            ...
            return log("Yapılandırma başarıyla yenilendi.", "i")
        } catch (error) {
            console.log(error);
            return log("Yapılandırma yenileme başarısız oldu.", "e")
        }
    }
}
...
async function aSleep() {
    ...
    log("Çok Çalıştım Azıcık Dinleceğim", "i")
...
// Belirlediğiniz kelimeleri buraya ekleyin
...
export async function notify(message, solved = false) {
    log(`Kanaldaki CAPTCHA bulundu:" #${message.channel.name}`, "a")
    const content = `${global.config.userNotify ? `<@${global.config.userNotify}> ` : " "}Kanaldaki CAPTCHA bulundu:" <#${message.channelId}>`
    ...
    if(global.config.wayNotify.includes(3)) {
        try {
            ...
            console.log(error);
            log("Müzik çalmak başarısız oldu.", "e");
        }
    }
    ...
    if(global.config.wayNotify.includes(0)) {
        ...
        if(message.attachments) {
            embed = new MessageEmbed()
                .setTitle("Selfbot, sadece CAPTCHA cevaplarını DM'lerle gönderiyor.")
                .setDescription(solved ? "ÇÖZÜLDÜ" : "**ÇÖZÜLEMEDİ**")
                .setColor("#0099ff")
